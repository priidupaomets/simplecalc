...
calc.Display = function () {
    var $displayControl,
        ...
        // This is the function that performs acual calculation based on the accumulated value and current number
        calculate = function () {
...
calc.calculator = function () {
    ...
    // Initialize the Display object inside our calc object and point it to the text input box and get the initial value displayed there
    calc.Display.init($("#displayPanel")[0]);
    
    // For each HTML element with a class of "key", attach "touchstart" and "click" for mouse event handlers
    $(".key").on('touchstart click', function (event) {
        
        // Extract key's value that should be added to the calculation; also get id of the button
        var key = $(this).attr("data-calc-tag"),
            id = this.id;

        console.log("Setting " + id + " with value "+ key);
        
        // this is a performance boost where we prevent default event handling and propagation.
